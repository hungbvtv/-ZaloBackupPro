...
- (void)startBackupFlow {
  ...
  [ac addAction:[UIAlertAction actionWithTitle:@“Luu vao Documents” style:UIAlertActionStyleDefault handler:^(UIAlertAction **) {
  ...
  [ac addAction:[UIAlertAction actionWithTitle:@“Chon Thu Muc (Files)…” style:UIAlertActionStyleDefault handler:^(UIAlertAction **) {
  NSString *name = ac.textFields.firstObject.text;
  if (!name.length) name = @“backup”;
  ...
  ```
    ...
    for (NSString *sp in search) {
        ...
        while ((f = en.nextObject)) {
            ...
            // FIX: dùng __ để encode path separator
...
- (void)doRestore:(NSString *)name {
  ...
  ```
    for (NSString *f in [fm contentsOfDirectoryAtPath:[dir stringByAppendingPathComponent:@"DB"] error:nil]) {
        ...
        // FIX: decode __ -> / để khôi phục đúng path
...
// FIX: khai báo đúng kiểu pointer
static ZBWindow *_w;

// FIX: thêm đầy đủ **attribute**((constructor))
**attribute**((constructor))
static void Init() {
// FIX: thêm dấu * vào 2.0 * NSEC_PER_SEC
